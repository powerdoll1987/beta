...
def exp_power(data):
    ...
    ratio_tmp = 0
    res = sm.OLS(np.asarray(data.ix[:,'USGG10YR Index']),np.array(data.ix[:,'CO1 Comdty'])).fit()
...
def rolling(d,day,func,name):
    ...
    while start_id < len(d.index):
        print(start_id,d.index[start_id])
...
data = pd.read_excel('fund.xlsx')
data.set_index('Date', inplace=True)

#m = corr_withspx(data)
#m.to_excel("corr.xlsx")

data = data[['USGG10YR Index','CO1 Comdty']].pct_change().dropna()
mm = rolling(data,13,exp_power,'Beta')
dd = rolling(mm,50,z_score_f,'Z-score')
cc = pd.concat([mm,dd,data_non_ret['USGG10YR Index'].ix[dd.index],data_non_ret['CO1 Comdty'].ix[dd.index]],axis = 1,join = 'inner')
